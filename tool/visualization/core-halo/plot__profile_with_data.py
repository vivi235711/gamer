...
background_density_0 = (3*hubble0**2*omega_M0)/(8*np.pi*newton_G)
...
def plot_profile(path,name, core_is_true = True, NFW = True):
    ...
    if (NFW):
        ...
        plt.plot(x, fit_NFW.NFW_dens(x, (rho0, Rs)))
...
for idx in range(idx_start, idx_end+1, didx):
    ...
    plot_profile('./', 'gamer')
    plot_profile('/projectV/vivi235711/Gadget2/L_4.0_m22_0.1_N_256_seed_294002/plot_script/', 'gadget', False, False)

    plt.xscale("log")
    plt.yscale("log")
    plt.xlim(1e-2,1e3)
    plt.ylim(1e0,1e10)
    plt.ylabel('$\\rho(r)$ (Msun/kpc)')
