...
#  example for loading the center coords from the last row of the file CenterCoords
   ...
   r=$(awk "BEGIN{r=$r; time=$time; r=r*0.6732117*1e-3*1.5; print r}");
...
#  example for setting all parameters manually
   mpirun -map-by ppr:1:socket:pe=8 ./GAMER_ExtractProfile_spock -i $FILE_IN -o "$SUFFIX" -x $x -y $y -z $z  -S -V -r $r -G 3.7698604e-02 -L 1.05 -p -O -C
