...
df_vel = pd.read_csv('halo_velocity_%d'%halo, sep = '\s+' , header = 0, index_col = '#')
df_halo = pd.read_csv('Halo_Parameter_%d'%halo, sep = '\s+' , header = 0, index_col = '#')
...
plt.plot(time_a, (core_mass/ms_ws),':', color = 'gray', label = 'integrity')
